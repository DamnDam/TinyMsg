...
def create_app():
    ...
    def clean_db():
        with app.app_context():
            ...
            db.session.query(Message).filter(~Message.id.in_(ids_to_keep)).delete(
    ...
    def view_messages():
        ...
        return render_template(
            "messages.html", last_message_id=last_message_id, messages=messages
        )
        ...
        def generate(start_id):
            with app.app_context():
                while True:  # infinite loop to keep the connection alive
                    messages = (
                        Message.query.filter(Message.id > start_id)
